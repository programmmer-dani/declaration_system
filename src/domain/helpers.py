...
from domain.security.security import (
    secure_claim_data,
    secure_user_data,
    verify_existing_username,
)
...
def reset_users_password(session):
    if session["role"] in ["manager", "admin"]:
        ...
        user = print_and_select_from_list(
            formatted_users, "Select user to reset password ('b' = back): "
        )
        ...
        return
    log_event(
        "unauthorized users password reset attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
...
def view_logs(session):
    if session["role"] in ["manager", "admin"]:
        ...
        return
    log_event(
        "unauthorized logs view attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def edit_employee_account(session):
    if session["role"] == "manager":
        ...
        employee = print_and_select_from_list(
            formatted_employees, "Select employee to edit ('b' = back): "
        )
        ...
        key_to_update = print_and_select_from_list(
            keys, "Select key to update ('b' = back): "
        )
        updated_value = go_validate(
            f"Enter new value for {key_to_update}: ", find_validator(key_to_update)
        )
        ...
        log_event(
            "employee account edited",
            username_enc=session["username_enc"],
            additional_info=f"employee account edited (id: {decrypt_value(employee['user_id'])}: {key_to_update} updated to {decrypt_value(updated_value)}",
        )
        return
    log_event(
        "unauthorized employee account edit attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def set_claims_salary_batch(session, claim_id):
    if session["role"] in ["manager", "admin"]:
        salary_batch = go_validate(
            "Enter salary-batch (YYYY-MM): ", validate_salary_batch
        )
...
def search_claims(session):
    if session["role"] not in ["employee", "manager", "admin"]:
        log_event(
            "unauthorized claims search attempt",
            username_enc=session["username_enc"],
            is_suspicious=True,
        )
        raise Exception("Unauthorized access")
    if session["role"] == "employee":
        ...
        log_event(
            "invalid role claims search attempt",
            username_enc=session["username_enc"],
            is_suspicious=True,
        )
        raise Exception("Unauthorized access")
    ...
    log_event(
        "claims searched",
        username_enc=session["username_enc"],
        additional_info=f"claims searched for {needle}",
    )
...
def search_employees(session):
    if session["role"] in ["manager", "admin"]:
        ...
        needle = _normalize_search_text(input_search_term())
        matched = [
            r for r in employees if _employee_row_matches_partial_search(r, needle)
        ]
        log_event(
            "employees searched",
            username_enc=session["username_enc"],
            additional_info=f"employees searched for {needle}",
        )
        if not matched:
        ...
        return
    log_event(
        "unauthorized employee search attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def update_password(session):
    ...
    if session["role"] in ["employee", "manager"]:
        ...
        log_event(
            "password updated",
            username_enc=session["username_enc"],
            additional_info=f"password updated for user: {decrypt_value(session['username_enc'])}",
        )
        return
    log_event(
...
def edit_manager_account_as_admin(session):
    if session["role"] == "admin":
        ...
        manager = print_and_select_from_list(
            formatted, "Select manager to edit ('b' = back): "
        )
        ...
        log_event(
            "manager account edited",
            username_enc=session["username_enc"],
            additional_info=f"manager account edited (id: {manager_id}): {key_to_update} updated to {decrypt_value(updated_value)}",
        )
        return
    log_event(
        "unauthorized manager account edit attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def delete_manager_account_as_admin(session):
    if session["role"] == "admin":
        ...
        formatted = format_user_list(managers)
        manager = print_and_select_from_list(
            formatted, "Select manager to delete ('b' = back): "
        )
        manager_id = manager["user_id"]
        ...
        log_event(
            ...
            username_enc=session["username_enc"],
            additional_info=f"manager account deleted (id: {manager['user_id']})",
        )
        return
    log_event(
        "unauthorized manager account delete attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def edit_manager_account(session):
    if session["role"] == "manager":
        ...
        log_event(
            "manager account edited",
            username_enc=session["username_enc"],
            additional_info=f"manager account edited (username: {decrypt_value(session['username_enc'])}: {key_to_update} updated to {decrypt_value(updated_value)}",
        )
        return
    log_event(
        "unauthorized manager account edit attempt",
...
def delete_manager_account(session):
    if session["role"] == "manager":
        ...
        log_event(
            "manager account deleted",
            username_enc=session["username_enc"],
            additional_info=f"manager account deleted (id: {session['user_id']})",
        )
        return "logout"
    log_event(
        "unauthorized manager account delete attempt",
...
def delete_employee_account(session):
    if session["role"] == "manager":
        ...
        formatted_employees = format_user_list(employees)
        employee = print_and_select_from_list(
            formatted_employees, "Select employee to delete ('b' = back): "
        )
        employee_id = employee["user_id"]
        ...
        log_event(
            ...
            username_enc=session["username_enc"],
            additional_info=f"employee account deleted (id: {employee['user_id']})",
        )
        return
    log_event(
        "unauthorized employee account delete attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def delete_claim(session):
    if session["role"] == "employee":
        ...
        claim = print_and_select_from_list(
            formatted_claims, "Select claim to delete ('b' = back): "
        )
        ...
        log_event(
            "claim deleted",
            username_enc=session["username_enc"],
            additional_info=f"claim deleted (id: {claim_id})",
        )
        return
    log_event(
        "unauthorized claim delete attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def edit_claim(session):
    if session["role"] == "employee":
        ...
        formatted_claims = format_claim_list(claims)
        claim = print_and_select_from_list(
            formatted_claims, "Select claim to edit ('b' = back): "
        )
        claim_id = claim["claim_id"]
        ...
        try:
            ...
            log_event(
                "Claim edit failed",
                username_enc=session["username_enc"],
                additional_info=f"Claim edit failed: {e}",
            )
            return
        log_event(
            "claim edited",
        ...
        return
    log_event(
        "unauthorized claim edit attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def edit_claim_as_manager_or_admin(session):
    if session["role"] in ["manager", "admin"]:
        ...
        log_event(
            "claim edited",
            username_enc=session["username_enc"],
            additional_info=f"claim edited (id: {claim_id}): {key_to_update} updated to {decrypt_value(updated_value)}",
        )
...
def find_validator(key_to_update):
    ...
    if key_to_update not in validators:
        log_event(
            "No validator found",
            is_suspicious=True,
            additional_info=f"No validator found: {key_to_update}",
        )
...
def is_key_value_encrypted(key_to_update):
    ...
    if key_to_update in encrypted_keys or key_to_update in [
        f"{key}_enc" for key in encrypted_keys
    ]:
        return True
    return False
...
def get_keys_to_update(claim_type, role=None):
    if role == "manager_or_admin":
        if claim_type == "Travel":
            return ["project_number", "travel_distance"]
        else:
...
def approve_claim(session):
    if session["role"] in ["manager", "admin"]:
        ...
        formatted_claims = format_claim_list(claims)
        claim = print_and_select_from_list(
            formatted_claims, "Select claim to approve ('b' = back): "
        )
        try:
            set_claims_salary_batch(session, claim["claim_id"])
            log_event(
                "claims salary batch  set",
                username_enc=session["username_enc"],
                additional_info=f"claim (id: {claim['claim_id']}) salary batch set during approve",
            )
        except Exception as e:
            print_error(f"Error setting salary-batch")
            log_event(
                "Claim salary batch set failed",
                username_enc=session["username_enc"],
                additional_info=f"Claim salary batch set failed: {e}",
            )
            return
        save_approved_claim(
            claim["claim_id"],
            get_user_id_by_username(decrypt_value(session["username_enc"])),
        )
        log_event(
            "claim approved",
            username_enc=session["username_enc"],
            additional_info=f"claim approved (id: {claim['claim_id']})",
        )
        print("\nClaim approved successfully.")
        return
    log_event(
        "unauthorized claim approve attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def reject_claim(session):
    if session["role"] in ["manager", "admin"]:
        ...
        formatted_claims = format_claim_list(claims)
        claim = print_and_select_from_list(
            formatted_claims, "Select claim to reject ('b' = back): "
        )
        save_rejected_claim(
            claim["claim_id"],
            get_user_id_by_username(decrypt_value(session["username_enc"])),
        ...
        log_event(
            "claim rejected",
            username_enc=session["username_enc"],
            additional_info=f"claim rejected (id: {claim['claim_id']})",
        )
        print("\nClaim rejected successfully.")
        return
    log_event(
        "unauthorized claim reject attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def create_claim(session):
    if session["role"] == "employee":
        ...
        return
    log_event(
        "unauthorized claim create attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
    ...
    if session_role in ["manager", "admin"]:
        ...
        try:
            ...
            log_event(
                "create user success",
                username_enc=session["username_enc"],
                additional_info=f"user created: {user_data['username']}",
            )
            return
            ...
            log_event(
                "Create user failed",
                username_enc=session["username_enc"],
                additional_info=f"Create user failed: {e}",
            )
            return
    log_event(
        "invalid role create user attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def request_employees_claims(session):
    if session["role"] == "employee":
        ...
        return
    log_event(
        "unauthorized employees claims view attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
    ...
    return [
        {
            ...
            "name": decrypt_value(employee["first_name_enc"])
            + " "
            + decrypt_value(employee["last_name_enc"]),
        }
...
def view_employee_list(session):
    if session["role"] == "manager":
        ...
        return
    log_event(
        "unauthorized employee list view attempt",
        username_enc=session["username_enc"],
        is_suspicious=True,
    )
    raise Exception("Unauthorized access")
...
def view_employees_claims(session):
    if session["role"] == "manager":
        ...
        formatted_employees = format_user_list(employees)
        employee = print_and_select_from_list(
            formatted_employees, "Select employee ('b' = back): "
        )
        ...
        print_claim_list(claims)
        log_event(
            "employees claims viewed",
            username_enc=session["username_enc"],
            additional_info=f"employee: {employee['user']} claims viewed",
        )  # employee is formatted without containing username_enc
        return
    log_event(
        "unauthorized employees claims view attempt",
        username_enc=session["username_enc"],
